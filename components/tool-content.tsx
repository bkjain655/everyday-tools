...
import { toolsMeta } from "@/lib/tools-meta"
 ...
 * ad-network content review. Resolved from the content registry by href.
 */
export function ToolContent({ href }: { href: string }) {
  const content = TOOL_CONTENT[href]
        ...
        {content.related.length > 0 && (
          <div>
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
              {content.related.map((relHref) => {
                const rel = toolsMeta.find((t) => t.href === relHref)
                if (!rel) return null
                return (
                  <Link key={relHref} href={rel.href} className="group block">
